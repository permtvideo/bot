...
while True:
    if (datetime.now().second == 0) and ((datetime.now().hour >=8) or (datetime.now().hour <=0)):
        ...
        for i in range(testarray.__len__()):
            if (int(datetime.now().timestamp()) - testarray[i][3]) >= 5400:
                ...
                if testarray[i][5] == testarray[i][2]:
                    ...
                    bot.send_message(chat_id=testarray[i][1], text=str(testarray[i][0]) + ' ' + 'https://t.me/' + testarray[i][2] , reply_to_message_id=testarray[i][0])
                conn.commit()
        conn.close()
