    ...
    try:
        ...
        def start(message):
                if message.text == '/start' or message.text == '/help':
                    bot.send_message(chat_id=message.chat.id, text='Любое отправленное сообщение будет запомнено и возвращено Вам через полтора часа. Если необходимо напомнить через другое время, то надо ответить на мое сообщение, в котором содержится ссылка на запомненную информацию, числом (можно использовать дробное число, например 0,5) и указать время в часах, начиная от текущего момента. После срабатывания напоминания, оно перейдет в режим ожидания на следующие полтора часа, время снова можно изменить, ответив на мое сообщение. Также можно запоминать пересланные из других чатов сообщения, тогда я напомню вам в каком чате требуется ваш ответ. Чтобы удалить напоминание, ответьте мне любым символом, кроме числа. Для вызова справки используйте команду /start или /help.')
                else:
